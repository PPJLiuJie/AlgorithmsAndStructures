...
与线性表、树、图等结构不同，散列技术的记录(元素)之间不存在什么逻辑关系， 它只与关键字有关联。 因此， 散列主要是面向查找的存储结构。

散列技术最适合的求解问题是查找与给定值相等的记录。 对于查找来说， 简化了比较过程， 效率就会大大提高。 但散列技术不具备很多常规数据结构的能力。

散列技术不适合一对多的情况。比如一个班级的学生有男有女，如果用关键字“男”去查找，对应的有许多的学生记录。如果用学号或者身份证号来散列存储，此时一个号码唯一对应一个学生才比较合适。

散列技术不适合范围查找。比如查找一个班级18～22岁的同学，在散列表中没法进行。 想获得表中记录的排序也不可能， 像最大值、 最小值等结果也都无法从散列表中计算出来。
